...
type CreativeRow = {
  ...
  title: string;
  description: string;
  type: "image" | "video" | "carousel" | "copy" | "headline";
  format: string;
  clientName: string;
  clientSlug: string;
  campaignName: string;
  ...
  return (
    <div className="space-y-5">
        ...
        {STAT_KEYS.map((key) => (
          <div key={key} className="ui-card p-3">
            ...
            <div className="mt-1 text-lg font-bold text-slate-900">
              {key === "total" ? counts.all.toLocaleString(locale) : "—"}
            </div>
            {key === "total" ? (
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <select className="ui-select w-auto min-w-[140px]">
          <option>{t("filterCampaign")}</option>
        </select>
          ...
          <div className="flex flex-wrap gap-1 border-b border-slate-200">
            {(
              [
                ...
                ["active", t("tabActive", { count: counts.active })],
                ["testing", t("tabTesting", { count: counts.testing })],
                ["paused", t("tabPaused", { count: counts.paused })],
                ["archived", t("tabArchived", { count: counts.archived })]
              ] as const
            ).map(([key, label]) => (
              <button
                key={key}
                  ...
                  {paged.map((row) => (
                    ...
                    >
                      <td className="px-3 py-3">
                        <input type="checkbox" className="accent-violet-600" onClick={(e) => e.stopPropagation()} />
                      </td>
                      <td className="px-3 py-3">
