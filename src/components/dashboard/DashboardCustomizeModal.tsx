  ...
  return (
    <>
      ...
      >
        <section className="campaign-creator-card campaign-creator-card--compact">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {DASHBOARD_AVAILABLE_SECTION_KEYS.map((key) => (
              <label
                key={key}
                className="flex cursor-pointer items-center gap-3 rounded-xl border px-3 py-2.5 transition-colors hover:bg-[var(--creator-card-bg-inset,var(--surface-bg))]"
        ...
        <section className="campaign-creator-card campaign-creator-card--compact">
          <h3 className="campaign-creator-orion-section-label mb-2">{t("layoutOrderTitle")}</h3>
        ...
        <section className="campaign-creator-card campaign-creator-card--compact">
          <div className="mb-1 flex items-center justify-between gap-2">
            <h3 className="campaign-creator-orion-section-label">{t("layoutHeroTitle")}</h3>
        ...
        <section className="campaign-creator-card campaign-creator-card--compact">
          <div className="mb-1 flex items-center justify-between gap-2">
            <h3 className="campaign-creator-orion-section-label">{t("layoutPeriodTitle")}</h3>
            <span className="text-[11px] text-[var(--text-dim)]">
              {t("layoutPeriodSelected", { count: draftLayout.periodMetrics.length })}
            </span>
          </div>
          <p className="mb-2 text-[11px] text-[var(--text-dim)]">{t("layoutPeriodHint")}</p>
          <input
            value={periodQuery}
          ...
          />
          <div
            className="max-h-36 overflow-y-auto rounded-xl border p-2"
            style={{ borderColor: "var(--creator-card-border, var(--border-color))" }}
          >
            {METRIC_CATEGORIES.map((cat) =>
              periodGrouped[cat].length ? (
                <div key={cat} className="mb-2 last:mb-0">
                  <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-[var(--text-dimmer)]">
                    {tCat(cat)}
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {periodGrouped[cat].map((m) => {
                      const checked = draftLayout.periodMetrics.includes(m.key);
                      const disabled = !checked && draftLayout.periodMetrics.length >= MAX_PERIOD_METRICS;
                      return (
                        <button
                          key={m.key}
                          type="button"
                          disabled={disabled}
                          onClick={() => togglePeriodMetric(m.key)}
                          className={`rounded-md px-2 py-1 text-[11px] font-medium transition-all ${
                            disabled ? "cursor-not-allowed opacity-40" : ""
                          }`}
                          style={
        ...
        <section className="campaign-creator-card campaign-creator-card--compact">
          <div className="mb-2 flex items-center justify-between gap-2">
            <div>
              <h3 className="campaign-creator-orion-section-label">{t("layoutChartTitle")}</h3>
        ...
        <section className="campaign-creator-card campaign-creator-card--compact">
          <h3 className="campaign-creator-orion-section-label mb-2">{t("layoutChartSizeTitle")}</h3>
