...
export function CopyCampaignModal({ open, onClose, clientSlug, selectedId, onSelect }: Props) {
  ...
  return (
    <CreatorModalShell
      open={open}
      onClose={onClose}
      title={t("copyCampaignSelect")}
      subtitle={t("copyCampaignHint")}
      titleIcon={<Copy size={16} />}
      width="md"
      ...
      {!clientSlug ? (
        ...
        <div className="space-y-3">
          ...
          {loading ? (
            ...
            <p className="py-6 text-center text-sm text-[var(--text-dim)]">{t("copyCampaignSelect")}</p>
          ) : (
