...
export function ClientMarketContextPanel({ clientId }: { clientId: string }) {
  ...
  if (loading) {
    return (
      <div className="ui-card p-4 text-sm text-slate-500">{t("loading")}</div>
    );
  }
  ...
  return (
    <div className="ui-card space-y-4 p-4">
      <div>
        <h3 className="font-semibold text-slate-900">{t("marketContextTitle")}</h3>
        <p className="mt-1 text-xs text-slate-500">{t("marketContextHint")}</p>
      </div>
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium text-slate-600">{t("marketContextCompetitors")}</p>
        {competitors.map((row, index) => (
          <div key={index} className="grid gap-2 rounded-lg border border-slate-200 bg-slate-50/80 p-2 sm:grid-cols-3">
            <input
