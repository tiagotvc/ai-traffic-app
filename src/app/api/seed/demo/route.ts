...
/** Gera métricas demo (30 dias) — só com DEMO_MODE=true. */
export async function POST() {
  ...
  const { defaultClient } = await getAppContext();
  const { adAccount: adAccountRepo, metricSnapshot: metricsRepo } = await repositories();

  let account = await adAccountRepo.findOne({ where: { clientId: defaultClient.id } });
  if (!account) {
    account = await adAccountRepo.save(
      adAccountRepo.create({
        clientId: defaultClient.id,
        metaAdAccountId: "act_demo",
        label: "Conta Demo"
      })
    );
  }
  ...
  return NextResponse.json({ ok: true, message: "Métricas demo geradas (30 dias)." });
}
