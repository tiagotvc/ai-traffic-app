...
import { confirmHypothesis } from "@/lib/agency-brain/hypothesis-service";
...
export async function PATCH(
  ...
  try {
    ...
    const hypothesis = await confirmHypothesis(tenant.id, client.id, hypothesisId);
    if (!hypothesis) {
    ...
    console.error("[hypothesis confirm]", err);
    return NextResponse.json({ ok: false, error: "Erro ao confirmar hipótese" }, { status: 500 });
  }
}
