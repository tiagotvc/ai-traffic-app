...
const BodySchema = z.object({
  ...
  deliveryChannel: z.enum(["email_pdf", "email_link", "whatsapp"]).default("email_pdf"),
  reportType: z.enum(["simple", "complete"]).default("simple"),
  periodPreset: z.string().max(40).nullable().optional(),
...
export async function POST(req: Request) {
  ...
  const row = await repo.save(
    repo.create({
      ...
      periodPreset: body.periodPreset ?? null,
